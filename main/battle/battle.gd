...
# Сигнал для завершения использования предмета
signal item_used
...
# --- Эффекты (Shake) ---
var rng = RandomNumberGenerator.new() # ИСПРАВЛЕНО: Добавлена декларация rng
...
func _ready() -> void:
	...
	# ИСПРАВЛЕНО: Логика инициализации инвентаря в бою
	if items_button:
		items_button.pressed.connect(_on_inventory_button_pressed)
	
	# Подключаем сигнал от самого себя (который кидает UI)
	if not self.item_used.is_connected(_on_item_used_finished):
		self.item_used.connect(_on_item_used_finished)

	# Пытаемся связать UI с данными инвентаря, если они есть в менеджере
	if inventory_ui and BattleManager.get("player_inventory"):
		inventory_ui.set_inventory(BattleManager.player_inventory)
