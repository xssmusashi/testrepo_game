...
@export var fruits_sheet: Texture2D # Перетащи сюда fruits.png в инспекторе
@export var fruit_display_scale: float = 3.0 # Масштаб x3
...
func _process(delta: float) -> void:
	...
	_update_slash_trail()
	if time_left <= 0: _finish()

func _update_slash_trail() -> void:
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
		var m_pos = get_local_mouse_position()
		trail.add_point(m_pos)
		if trail.points.size() > 15: trail.remove_point(0)
	else:
		if trail.points.size() > 0: trail.remove_point(0)
...
func _finish() -> void:
	is_active = false
	spawn_timer.stop()
	finished.emit({"success": score >= 5, "damage": 0 if score >= 5 else 12})
	visible = false
